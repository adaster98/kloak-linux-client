(() => {
  ...
  async function loadConfig() {
    try {
      ...
      console.error("[Hover Zoom] Failed to load config", e);
    }
  }
  ...
  const createZoomUI = (imgElement) => {
    ...
    Object.assign(zoomedImg.style, {
      ...
      maxWidth: "none",
      maxHeight: "none", // Prevent global styles from squishing it
    });
    ...
    renderSettings: async (container) => {
      ...
      container.innerHTML = `
            ...
            <div class="addon-settings-item">
                ...
                <input id="hz-size-input" type="range" min="150" max="600" value="${config.lensSize}" class="invisic-slider hz-slider">
            </div>
            <div class="addon-settings-item">
                <label class="addon-label">Magnification <span class="addon-val"><span id="hz-zoom-val">${config.zoomLevel}</span>x</span></label>
                <input id="hz-zoom-input" type="range" min="1.1" max="10" step="0.1" value="${config.zoomLevel}" class="invisic-slider hz-slider">
            </div>
      ...
      const saveBtn = container.querySelector("#hz-save-btn");
      ...
      [sizeIn, zoomIn].forEach((el) => {
        updateSliderFill(el);
        el.addEventListener("input", (e) => {
          const valId =
            e.target.id === "hz-size-input" ? "#hz-size-val" : "#hz-zoom-val";
          container.querySelector(valId).textContent = e.target.value;
      ...
      saveBtn.onclick = () => {
        ...
        saveBtn.textContent = "✓ Calibration Saved";
